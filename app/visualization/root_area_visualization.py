...
class VisualizationInitWorkerArea(QThread):
    ...
    def run(self):
        try:
            logger.debug(f"Worker thread: Initializing DataProcessorArea with csv_path={self.csv_path}")
            processor = DataProcessorArea(self.csv_path)
            
            logger.debug(f"Worker thread: DataProcessorArea initialized, df shape: {processor.df.shape}")
            if processor.df.empty:
                raise ValueError("No data found in CSV file")
            
            logger.debug("Worker thread: Creating DashAppArea")
            dash_app = DashAppArea(processor, self.save_directory)
            logger.info("Worker thread: DashAppArea created successfully")
...
class DashServerThreadArea(QThread):
    error = pyqtSignal(str)
    port_assigned = pyqtSignal(int)

    def __init__(self, dash_app, port=8051):
        super().__init__()
        self.dash_app = dash_app
        self.server = None
        self.port = port
        self._stop_event = threading.Event()
        logger.debug(f"DashServerThreadArea initialized with port {port}")

    def run(self):
        logger.debug(f"DashServerThreadArea.run() started for port {self.port}")
        try:
...
class RootAreaVisualization(QMainWindow):
    ...
    def __init__(self, csv_path):
        logger.info(f"RootAreaVisualization.__init__ called with csv_path={csv_path}")
        super().__init__()
        self.csv_path = csv_path
        ...
        self.port = 8051  # Use different port than length visualization
    ...
    def _on_init_finished(self, processor, dash_app):
        ...
        try:
            ...
            logger.debug(f"Creating DashServerThreadArea on port {self.port}")
